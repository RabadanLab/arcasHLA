...
def predict_genotype(eqs, allele_idx, allele_eq, em_results, gene_count, 
    ...
    if len(em_results) > 1:
        ...
        log.info('\t\t{: <28}\t{: >7}\t'.format('allele pair', '% reads'))
        ...
        if a1_count == a2_count == 0:
            log.info('[genotype] Likely heterozygous: unable to distinguish ' +
...
def arg_check_tolerance(parser, arg):
    if arg > 1 or arg < 0:
        parser.error(f'The tolerence {arg} is invalid.')
    return arg
    
def arg_check_iterations(parser, arg):
    if type(arg) != int or arg < 0:
        parser.error(f'The iterations {arg} is invalid.')
    return arg
    
def arg_check_threshold(parser, arg):
    if arg > 1 or arg < 0:
        parser.error(f'The threshold {arg} is invalid.')
    return arg
...
if __name__ == '__main__':
    ...
    log.info('[alignment] Processed {:.0f} reads, {:.0f} pseudoaligned to HLA reference'
    ...
    for gene in args.genes:
        ...
        log.info(f'[genotype] {gene_count:.0f} reads aligned to HLA-{gene} in {eq_count} classes')
